...
# is slow. Second, we need to append a file list to nsxiv, which can only be done
...
# NOTES:
# This is like rifle_nsxiv.sh but changes first to the directory of the 1st
...
listfiles () {
    find -L . -maxdepth 1 -type f -iregex \
      '.*\(jpe?g\|bmp\|png\|gif\|tga\|xpm\|webp\|svg\)$' -print0 | sort -z
}

count="$(listfiles | grep -m 1 -ZznF "./${selected_img}" | cut -d: -f1)"

if [ -n "$count" ]; then
    listfiles | xargs -0 nsxiv -ab -n "$count" -- &
else
    cd - || exit 1
    nsxiv -ab -- "$@" & # fallback
fi
